...
class VirtualTB(gym.Env):
    ...
    def step(self, action):
        ...
        return self.state, reward, done, {'CTR': self.total_a / self.total_c / self.n_recommended_item}
    ...
    def render(self, mode='human', close=False, short=False):
        a, b = np.clip(self.rend_action, a_min = 0, a_max = None)
        if short:
            s = f"{''.join(hex(int(''.join(str(int(x)) for x in self.state[i:i+4]), 2))[2:] for i in range(0, self.n_user_feature, 4))} {int(self.state[-3]):>2} {int(self.state[-2])} {int(self.state[-1]):>3}"
            if mode == 'return':
                return s
            print(s)
        else:
            s = f"Current State:\n" + \
                f"\t {self.state}\n" + \
                f"User\'s action:\n" + \
                f"\tclick:{int(a):>2d}, max_index_reached:{'True' if self.total_c > self.max_c else 'False'}, index:{int(self.total_c):>2d}\n" + \
                f"Total clicks: {self.total_a}"
            if mode == 'return':
                return s
            print(s)
    ...
    def __str__(self):
        string = f"<TBEnv - action_click:{self.rend_action}; index:{self.total_c}; total_clicks:{self.total_a}>"
        return string
