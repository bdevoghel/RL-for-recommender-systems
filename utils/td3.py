# from https://github.com/philtabor/Youtube-Code-Repository/blob/master/ReinforcementLearning/PolicyGradient/TD3/td3_torch.py
...
class Agent():
    ...
    def choose_action(self, observation, time_step):
        if time_step < self.warmup:
    ...
    def get_q(self, observation, action):
        state = T.tensor(np.expand_dims(observation, axis=0), dtype=T.float)#.to(self.actor.device)
        action = T.tensor(np.expand_dims(action, axis=0), dtype=T.float)#.to(self.actor.device)
